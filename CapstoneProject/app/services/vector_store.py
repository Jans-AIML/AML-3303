...
def _clean_chunk(text: str) -> str:
    """Remove NIH watermark boilerplate from within a retrieved chunk."""
...
def retrieve_chunks(query: str, top_k: int | None = None) -> tuple[list[str], list[dict]]:
    """Return top-k relevant chunks and structured source metadata dicts.

    Each source dict contains: filename, doc_id, chunk_index, title, authors,
    pub_year, doi, subjareas, chunk_text — matching the SourceItem schema.
    """
    collection = _get_collection()
    ...
    # Fallback: include anything non-empty if all were filtered out
    if not candidates:
        for doc, meta in zip(docs, metas):
            if doc:
                candidates.append((doc.strip(), meta))

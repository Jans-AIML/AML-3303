...
if st.session_state["page"] == "📊 Analytics":
    ...
    analytics = cached_api_get("/api/documents/analytics")
    if analytics:
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("📄 Documents", f"{analytics['total_documents']:,}")
        c2.metric("🔢 Chunks Indexed", f"{analytics['total_chunks']:,}")
        c3.metric("💬 Chat Sessions", analytics["total_sessions"])
        c4.metric("❓ Queries Answered", analytics["total_queries"])
    else:
